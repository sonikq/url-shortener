...
func customTimeEncoder(t time.Time, encoder zapcore.PrimitiveArrayEncoder) {
	encoder.AppendString(t.Format(customTimeFormat))
}
...
// GeZapLogger extracts zap struct from given logger interface
func GeZapLogger(l Logger) *zap.Logger {
	if l == nil {
		return newZapLogger(LevelInfo, time.RFC850)
	}
	...
	default:
		l.Info(
			...
			String("time_format", time.RFC850),
		)
		return newZapLogger(LevelInfo, time.RFC850)
	}
}
